...
from invariant.language.scope import ExternalReference
...
def resolve(ref: ExternalReference):
    ...
    try:
        ...
        raise ImportError(f"Module '{module_name}' could not be resolved (stdlib path: {os.path.abspath(STDLIB_PATH)})") from None
