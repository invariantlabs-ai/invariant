        ...
        def decorator(func):
            ...
            def wrapped(tool_input, *args, **kwargs):
                if not isinstance(tool_input, dict):
                    raise ValueError(f"Expected a dictionary of all tool parameters, but got: {tool_input} (note that @tool functions must be called with a dictionary of arguments and do not support positional arguments)")
                ...
                assert len(tool_call_msg["tool_calls"]) == 1, f"Expected a single tool call: {tool_call_msg}"
                tool_call = tool_call_msg["tool_calls"][0]
                assert tool_call["function"]["name"] == name, f"Expected a tool call to {name} as last message, but got: {tool_call}"
