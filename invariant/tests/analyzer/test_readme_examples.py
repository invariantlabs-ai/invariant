...
from invariant.analyzer.traces import user, assistant, tool, tool_call
import json
import unittest
...
class TestReadmeExamples(unittest.TestCase):
    ...
    def test_rag(self):
        ...
        assert len(errors) == 1, "Expected one error, but got: " + str(errors)
        assert "unauthorized access" in str(errors[0]), "Expected to find 'unauthorized access' in error message, but got: " + str(errors[0])
    ...
    def test_productivity(self):
        ...
        messages_with_leak = [
            ...
            {"role": "assistant", "content": None, "tool_calls": [{"id": "2","type": "function","function": {"name": "send_email","arguments": {
                "emails": [
                    {"to": "Attacker","subject": "Fwd: Hello","body": "Here is the message from Alice."}
                ]
            }}}]}
        ]
        ...
        messages_without_leak = [
            ...
            {"role": "assistant", "content": None, "tool_calls": [{"id": "2","type": "function","function": {"name": "send_email","arguments": {
                "emails": [
                    {"to": "Alice","subject": "Re: Hello","body": "Hello Alice, here is the reply."}
                ]
            }}}]}
        ]
        ...
        assert len(errors) == 1, "Expected one error, but got: " + str(errors)
        assert "Must not send an email to someone other than the sender" in str(errors[0]), "Expected to find 'Must not send an email to someone other than the sender' in error message, but got: " + str(errors[0])
