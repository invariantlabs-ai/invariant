...
def format_message(idx, message, arg_value_width=0, **kwargs):
    ...
    if "tool_calls" in message and len(message["tool_calls"]) > 0:
        ...
        if 'invariant_highlight' in message["tool_calls"][0]:
            ...
            style = lambda x: x
        arg = ",".join([f"{k.strip()}:{clip_string(v.strip(), width=arg_value_width)}" for k,v in arg.items()])
...
class TraceSet:
    ...
    def filter(self, invariant_condition: str, max_items: int = None, python:str = None, prefix:str = None) -> "TraceSet":
...
{textwrap.indent(invariant_condition, "  ")}
    ...
    def pretty(self, max_lines=30, **kwargs):
        ...
        if max_lines > 1 and len(self.traces) > max_lines:
            ...
            traces = self.traces
        traces = [format_trace(t, join_sequence=' -> ', max_content_lines=1, arg_value_width=5, **kwargs) for t in traces]
        traces = [termcolor.colored(f"Trace {i}: ", attrs=["bold"]) + trace for i, trace in enumerate(traces)]
...
class OpenDevinLoader(TraceSet):
    ...
    def parse_trace(trajectory):
        from invariant.traces import user, tool, tool_call, assistant
        import re
...
class SWEAgentTraceSet(TraceSet):
    ...
    @staticmethod
    def parse_trace(trajectory):
        from invariant.traces import tool, tool_call, assistant
        inv_traj = []
