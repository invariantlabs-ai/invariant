from invariant.analyzer.policy import Policy, PolicyRoot, Input, UnhandledError, parse, parse_file, AnalysisResult
...
class Monitor(Policy):
    ...
    def run(self, operation: ValidatedOperation):
        ...
        analysis_result.execute_handlers()
        wrappers = [partial(handler_call.handler, error=handler_call.error) for handler_call in analysis_result.handled_errors if is_wrapping(handler_call.handler)]
...
def stack_functions(remaining_calls=[]):
    ...
    if not "call_next" in inspect.signature(call).parameters:
        return call
...
def wrappers(analysis_result: AnalysisResult):
    ...
    """
    return [partial(handler_call.handler, error=handler_call.error) for handler_call in analysis_result.handled_errors if is_wrapping(handler_call.handler)]